...
interface MeetingState {
  ...
  triggerCopilotId: string | null;
  manualDirective: string | null;
  isSidebarCollapsed: boolean;
  liveSuggestion: string | null;
...
type Action =
  ...
  | { type: 'TRIGGER_COPILOT';     id:        string }
  | { type: 'SET_MANUAL_DIRECTIVE'; directive: string | null }
  | { type: 'SET_SIDEBAR_COLLAPSED'; collapsed: boolean }
  | { type: 'SET_LIVE_SUGGESTION';   suggestion: string | null }
  ...
  switch (action.type) {
    ...
    case 'SET_STRATEGIC_BRIEF':
      return {
        ...
        isSidebarCollapsed: true, // Auto-collapse when brief is generated
      };
    case 'SET_STREAMING':
    ...
    case 'TRIGGER_COPILOT':
      return { ...state, triggerCopilotId: action.id };
    case 'SET_MANUAL_DIRECTIVE':
      return { ...state, manualDirective: action.directive };
    case 'SET_SIDEBAR_COLLAPSED':
      return { ...state, isSidebarCollapsed: action.collapsed };
    case 'SET_LIVE_SUGGESTION':
      return { ...state, liveSuggestion: action.suggestion };
...
interface MeetingStore extends MeetingState {
  ...
  triggerCopilot:        (id: string) => void;
  setManualDirective:    (directive: string | null) => void;
  setSidebarCollapsed:   (collapsed: boolean) => void;
  setLiveSuggestion:     (suggestion: string | null) => void;
  setInterimTranscript:  (text: string) => void;
  startMeeting:          () => void;
  stopMeeting:           () => void;
  fireLiveSuggestion:    (recentText: string) => Promise<void>;
...
export function MeetingProvider({ children }: { children: ReactNode }) {
  ...
  const triggerCopilot = useCallback((id: string) => {
    dispatch({ type: 'TRIGGER_COPILOT', id });
  }, []);

  const setManualDirective = useCallback((directive: string | null) => {
    dispatch({ type: 'SET_MANUAL_DIRECTIVE', directive });
  }, []);
  ...
  const setLiveSuggestion = useCallback((suggestion: string | null) => {
    dispatch({ type: 'SET_LIVE_SUGGESTION', suggestion });
  }, []);
  ...
  const fireLiveSuggestion = useCallback(async (recentText: string) => {
    dispatch({ type: 'SET_STREAMING', streaming: true });
    dispatch({ type: 'SET_LIVE_SUGGESTION', suggestion: '' }); // Clear old

    try {
      const baseUrl = process.env.NEXT_PUBLIC_API_URL || '';
      const res = await fetch(`${baseUrl}/api/ai/live`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ 
          recentTranscript: recentText,
          prospectData: stateRef.current.prospectData,
          currentPhase: stateRef.current.currentPhase,
          strategicBrief: stateRef.current.strategicBrief
        }),
      });
      ...
      while (true) {
        ...
        streamedText += chunk;
        dispatch({ type: 'SET_LIVE_SUGGESTION', suggestion: streamedText });
      }
  ...
  const fireOverride = useCallback(async (action: TacticalAction) => {
    ...
    const recentLines = stateRef.current.transcript
    ...
    try {
      const baseUrl = process.env.NEXT_PUBLIC_API_URL || '';
      const res = await fetch(`${baseUrl}/api/ai/override`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ 
          action, 
