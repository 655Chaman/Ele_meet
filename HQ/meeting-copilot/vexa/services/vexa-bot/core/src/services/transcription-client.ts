...
export class TranscriptionClient {
  ...
  private async sendRequest(wavBuffer: Buffer, language?: string, prompt?: string): Promise<TranscriptionResult> {
    ...
    // Detect external APIs early — needed for model name and feature gating
    const isGroq = this.serviceUrl.includes('api.groq.com');
    const isOpenAI = this.serviceUrl.includes('api.openai.com');
    const isExternal = isGroq || isOpenAI;
    ...
    // Request word-level timestamps (OpenAI supports it, Groq does not)
    ...
    if (this.maxSpeechDurationSec !== undefined && !isExternal) {
      parts.push(Buffer.from(
        `--${boundary}\r\n` +
    ...
    if (this.minSilenceDurationMs !== undefined && !isExternal) {
      parts.push(Buffer.from(
        `--${boundary}\r\n` +
