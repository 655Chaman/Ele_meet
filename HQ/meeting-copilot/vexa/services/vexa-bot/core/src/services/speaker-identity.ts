...
async function resolveGoogleMeetSpeakerName(
  ...
  // Query browser for currently speaking participants via native Google Meet DOM indicators
  const state = await queryBrowserState(page, botName);
  if (!state) return null;

  const { speaking } = state;

  // In Google Meet, WebRTC tracks may be multiplexed for efficiency in large meetings.
  // A permanent lock on an element index causes misattribution if the SFU swaps streams.
  // Instead, we dynamically correlate the active track with Google Meet's native active speaker indicators.

  // Single speaker → 100% confidence dynamic attribution
  if (speaking.length === 1) {
    const candidate = speaking[0];
    recordTrackVote(elementIndex, candidate, 1.0); // Keep historical record for fallback
    return candidate; 
  }

  // Multiple speakers overlapping → partial vote
  if (speaking.length >= 2) {
    for (const candidate of speaking) {
      recordTrackVote(elementIndex, candidate, 0.5);
    }
    
    // Resolve tie by returning the historically most-correlated name for this track
    // THAT is currently in the 'speaking' array.
    const votes = trackVotes.get(elementIndex);
    if (votes && votes.size > 0) {
      const sorted = Array.from(votes.entries()).sort((a, b) => b[1] - a[1]);
      for (const [name] of sorted) {
        if (speaking.includes(name)) return name;
      }
    }
    
    // Fallback to first speaking candidate
    return speaking[0];
  }

  // Zero speakers (silence or temporary DOM mismatch) -> fallback to history
  const votes = trackVotes.get(elementIndex);
  if (votes && votes.size > 0) {
    const sorted = Array.from(votes.entries()).sort((a, b) => b[1] - a[1]);
    return sorted[0][0]; // Most historically likely speaker for this track
  }
