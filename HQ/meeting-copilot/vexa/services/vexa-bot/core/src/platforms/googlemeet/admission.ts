...
export async function waitForGoogleMeetingAdmission(
  ...
  // Cap the waiting room timeout to 5 minutes (300,000 ms). 
  // If the host is AFK, we gracefully exit with code 0 to free up compute resources.
  timeout = Math.min(timeout || 300000, 300000);

  try {
    log(`Waiting for Google Meet meeting admission (timeout capped at ${timeout}ms)...`);
    ...
    if (stillInWaitingRoom) {
      ...
      while (Date.now() - startTime < effectiveTimeout2()) {
        ...
        // If state remains unknown for too long, check for escalation
