...
async function handlePerSpeakerAudioData(speakerIndex: number, audioDataPayload: number[] | string): Promise<void> {
  ...
  if (typeof audioDataPayload === 'string') {
    // Base64 decode for high-performance Worklet IPC
    const binary = Buffer.from(audioDataPayload, 'base64');
    audioData = new Float32Array(binary.buffer, binary.byteOffset, binary.byteLength / 4);
  } else {
    // Fallback for legacy number[] array
    audioData = new Float32Array(audioDataPayload);
  }
...
export async function startPerSpeakerAudioCapture(pageToCaptureFrom: Page): Promise<void> {
  ...
  const handleCount = await pageToCaptureFrom.evaluate(async () => {
    ...
    // Inject AudioWorkletProcessor to prevent blocking the main thread
    const workletCode = `
      class VexaAudioProcessor extends AudioWorkletProcessor {
        constructor() {
          super();
          this.targetBufferLength = 16000 * 0.5; // 500ms chunking
          this.buffer = new Float32Array(this.targetBufferLength);
          this.bufferIndex = 0;
          
          this.preRollLength = 16000 * 0.3; // 300ms pre-roll silence buffer
          this.preRollBuffer = new Float32Array(this.preRollLength);
          this.preRollIndex = 0;
        ...
        float32ToBase64(f32Array) {
          const uint8Array = new Uint8Array(f32Array.buffer, f32Array.byteOffset, f32Array.byteLength);
          let binary = '';
          for (let i = 0; i < uint8Array.length; i+=1024) {
            binary += String.fromCharCode.apply(null, uint8Array.subarray(i, Math.min(i + 1024, uint8Array.length)));
          }
          return btoa(binary);
        }
        ...
        process(inputs, outputs, parameters) {
          ...
          if (maxVal > 0.005) { // VAD Gate breach
            if (!this.isSpeaking) {
              this.isSpeaking = true;
              // Send pre-roll buffer to prevent first-syllable clipping
              const preRollOut = new Float32Array(this.preRollLength);
              for (let i = 0; i < this.preRollLength; i++) {
                preRollOut[i] = this.preRollBuffer[(this.preRollIndex + i) % this.preRollLength];
              }
              this.port.postMessage({ type: 'audio', data: this.float32ToBase64(preRollOut) });
            }
            this.silenceFrames = 0;
          } else {
            if (this.isSpeaking) {
              this.silenceFrames += channelData.length;
              if (this.silenceFrames > 8000) { // 500ms silence closes gate
                this.isSpeaking = false;
                if (this.bufferIndex > 0) {
                   const finalBuffer = new Float32Array(this.buffer.buffer, 0, this.bufferIndex);
                   this.port.postMessage({ type: 'audio', data: this.float32ToBase64(finalBuffer) });
                   this.bufferIndex = 0;
                }
              }
            }
          ...
          if (this.isSpeaking) {
            for (let i = 0; i < channelData.length; i++) {
              ...
              if (this.bufferIndex >= this.targetBufferLength) {
                this.port.postMessage({ type: 'audio', data: this.float32ToBase64(this.buffer) });
                this.bufferIndex = 0;
              }
            }
          } else {
            for (let i = 0; i < channelData.length; i++) {
              this.preRollBuffer[this.preRollIndex] = channelData[i];
              this.preRollIndex = (this.preRollIndex + 1) % this.preRollLength;
            }
          }
      ...
      registerProcessor('vexa-audio-processor', VexaAudioProcessor);
    `;
    ...
    // Use a single shared AudioContext
    const sharedCtx = new AudioContext({ sampleRate: TARGET_SAMPLE_RATE });
    ...
    function connectElement(el: HTMLMediaElement, index: number): boolean {
      try {
        ...
        const workletNode = new AudioWorkletNode(sharedCtx, 'vexa-audio-processor');
        ...
        workletNode.port.onmessage = (event) => {
          if (event.data.type === 'audio') {
            ...
            streamLastActive.set(index, Date.now());
            // Send Base64 payload instead of raw array
            (window as any).__vexaPerSpeakerAudioData(index, event.data.data);
          }
        };

        source.connect(workletNode);
        workletNode.connect(sharedCtx.destination);
...
export async function runBot(botConfig: BotConfig): Promise<void> {// Store botConfig globally for command validation
  ...
  // Simple browser setup like simple-bot.js
    ...
    // Video IN (receive participant video): controlled by videoReceiveEnabled (default off)
    // When off, disables incoming video tracks to save ~87% CPU per bot.
    if (!botConfig.videoReceiveEnabled && botConfig.platform !== 'google_meet') {
      try {
        await context.addInitScript(getVideoBlockInitScript());
